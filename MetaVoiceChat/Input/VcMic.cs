...
namespace Assets.Metater.MetaVoiceChat.Input
{
    public class VcMic : IDisposable
    {
        ...
        public void StartRecording()
        {
            ...
            AudioClip = Microphone.Start(CurrentDeviceName, true, config.InputLoopSeconds, config.Frequency);
            ...
            recordCoroutine = config.CoroutineProvider.StartCoroutine(RoutineRecord());
        }
        ...
        private IEnumerator RoutineRecord()
        {
            int loops = 0;
            int readAbsPos = 0;
            int prevPos = 0;
            float[] segment = new float[config.SegmentLength];
